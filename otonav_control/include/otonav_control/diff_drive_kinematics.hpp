...
inline WheelSpeeds inverse_kinematics(
  const BodyTwist & twist, double wheel_radius, double track_width)
{
...
inline BodyTwist forward_kinematics(
  const WheelSpeeds & wheels, double wheel_radius, double track_width)
{
  const double r = wheel_radius;
  const double l = track_width;
  return BodyTwist{
    r * (wheels.right + wheels.left) / 2.0,
    r * (wheels.right - wheels.left) / l};
}
