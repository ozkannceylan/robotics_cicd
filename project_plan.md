...
**Goal:** A working end-to-end ROS 2 + MuJoCo Software-in-the-Loop (SIL) CI/CD platform on GitHub, built as a portfolio project demonstrating integration engineering capabilities.

**Strategic priority:** The project focuses on *integration* rather than core robotics development. It prioritizes PR lifecycle management, merge queues, repo hygiene, CI/CD, Docker, and middleware troubleshooting over complex navigation algorithms. Therefore:
...
| Sim | MuJoCo 3.3.x (pin exact version) | Industry-relevant, headless-friendly, no GPU needed for physics |
| ROS | ROS 2 Humble on Ubuntu 22.04 | LTS, matches doc baseline |
| Bridge language | C++ (rclcpp) | Production robotics demands solid C++; this is the showcase code |
...
| CI | GitHub Actions + native Merge Queue + branch protection | Mirrors real-world integration-engineering responsibilities |

## Phases

### Phase 0 — Repo & Hygiene Scaffold
The core infrastructure. All of it works before any robot code exists.
...
- Ensure `architecture.md` covers all core decisions
- Prepare a repo tour: PR → checks → merge queue → release tag → GHCR image
- Validate the PR pipeline with a live test

## Cut Lines (if time constrained)
Cut in this order: Phase 4 GHCR push → rosbag-on-failure artifact → obstacle avoidance (keep go-to-goal only). **Never cut Phase 0 or Phase 2** — they are the core focus.
